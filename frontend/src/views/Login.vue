    ...
    <div class="w-full max-w-md animate-fade-up relative z-10">
      ...
      <div class="glass rounded-2xl p-8 shadow-2xl shadow-gray-200/50 dark:shadow-black/50 dqs-auth-card">
        ...
        <form @submit.prevent="handleLogin" class="space-y-5">
          ...
          <template v-else>
            <div class="animate-slide-up" style="animation-delay: 100ms">
              <label class="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-2">Authenticator Code</label>
              <div class="relative">
                <div class="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400">
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/>
                ...
                <input v-model="totpCode" type="text" required placeholder="000 000" maxlength="7"
                  class="input-field pl-12 text-lg tracking-widest text-center" />
              </div>
              <p class="text-xs text-gray-500 mt-3 text-center cursor-pointer hover:text-gray-700" @click="requires2FA = false">Return to Login</p>
            </div>
          </template>
          ...
          <button type="submit" :disabled="loading" class="btn-primary w-full animate-slide-up" style="animation-delay: 300ms">
            ...
            <span v-else>{{ requires2FA ? 'Verify Code' : 'Sign In' }}</span>
          </button>
        </form>
        ...
        <div class="mt-6 text-center animate-fade-in" style="animation-delay: 400ms">
          <p class="text-gray-500 dark:text-gray-500 text-sm">
            ...
            <router-link to="/register" class="text-purple-500 hover:text-purple-400 font-medium transition-colors">Create one</router-link>
          </p>
        </div>
      </div>
    </div>
...
async function handleLogin() {
  ...
  try {
    ...
    if (e.response?.data?.detail === "totp_required") {
      requires2FA.value = true
    } else {
      error.value = e.response?.data?.detail || 'Login failed'
      if (requires2FA.value && error.value === "Invalid 2FA code") {
        totpCode.value = '' // clear code to retry
      }
    }
  } finally {
    loading.value = false
