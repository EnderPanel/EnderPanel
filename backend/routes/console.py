...
def get_recent_logs_text(name: str, tail: int = 100) -> str:
    client = get_docker_client()
    try:
        container = client.containers.get(name)
        logs = container.logs(tail=tail).decode("utf-8", errors="replace")
        return clean_console_text(logs, keep_ansi=True)
    except Exception:
        return ""


async def send_recent_logs(ws: WebSocket, name: str, tail: int = 100, started_at: str = "") -> None:
    text = await asyncio.to_thread(get_recent_logs_text, name, tail=tail)
    if text.strip():
...
async def console_stream(
    ...
    async def event_generator():
        try:
            ...
            if replay:
                replay_text = await asyncio.to_thread(get_recent_logs_text, name, tail=tail)
            ...
            while True:
                ...
                try:
                    readable, _, _ = await loop.run_in_executor(
                        None,
                        lambda: select.select([raw_sock], [], [], 1.0),
                    )
                    if not readable:
                        yield ": ping\n\n"
                        continue
    ...
    async def pump_output():
        while True:
            try:
                readable, _, _ = await loop.run_in_executor(
                    None,
                    lambda: select.select([raw_socket], [], [], 0.5),
                )
                if not readable:
                    continue
            except (ConnectionError, OSError):
